...
def remove_uploadedimage_and_image(apps, schema_editor):
    ...
    UploadedImage = apps.get_model('app', 'UploadedImage')
    UploadedImage.objects.all().delete()
    BlogPost = apps.get_model('app', 'BlogPost')
    BlogPost.objects.all().update(image=None)

class Migration(migrations.Migration):

    dependencies = [
        ('app', '0004_blogpost_publish'),  # Update with your actual previous migration
    ]
